...
**The Fix.** A sequencing skill, not a competing design methodology: it loads Anthropic's own `frontend-design` skill first for the actual typography/color/motion craft, adds a brand-facts-gathering step that reads real source files (the actual mark SVG, the actual token file — which wins over a written brand doc when they disagree) instead of inventing identity, a brief-structure template for dispatching the creative pass to an escalated model tier (`opus` by default, `fable` only on explicit request, dropping back to the current tier once the direction is locked for mechanical edits), a concrete anti-slop checklist of the specific patterns that read as generated rather than designed, and a verification step that actually opens the result in a browser — checking `prefers-reduced-motion`, responsive behavior, and both color schemes — rather than approving from the markup. Generalized from a project-specific skill built for one product's brand rules into a project-agnostic version any product can point at its own facts.

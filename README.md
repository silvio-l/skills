...
**The Problem.** Knowing the AI-design workflow is not the same as being able to produce something that doesn't look like every other AI-generated site or app. AI-driven design tools (Claude Design and others) move fast — the UI, model lineup, and available templates shift every few weeks — but the actual failure mode is upstream of any tool: an agent that never looks at what it built defaults to the same purple-gradient, four-identical-card, Inter-everywhere median every other AI output lands on, because nothing forced it to commit to a direction or check the result against it.

**The Fix.** An execution playbook, not just a workflow reference. `cool-craft.md` is the core capability: a table of the specific tells that give away generic AI output with a concrete fix for each, a mandatory direction-first gate (name one aesthetic direction and why it fits the subject *before* generating anything), a library of seven named directions (editorial, brutalist, retro-futuristic, maximalist, refined-minimal, organic-warm, technical/data) each with a concrete type/color/layout/motion recipe, litmus checks, and — the actual differentiator — a visual self-verify loop (build → render/screenshot → critique against the tells → iterate) so the agent isn't working blind. `patterns.md` backs it with copy-paste, CSP-safe code starters (token layer, bento grid, staggered scroll reveal, custom cursor, microinteractions) plus their Flutter translations. `assets.md` closes the last gap — generating real illustrations, icons, and hero imagery directly through the connected `fal.ai` MCP tools (model discovery → schema → pricing → run, with a hard cost-confirmation gate before any billed generation) instead of pointing the user elsewhere, plus when to reach for code-drawn SVG instead. `studio-workflow.md` and `motion-graphics.md` cover the durable Claude Design studio workflow (five artifact types, four motion-graphics methods, the prompt-precision lesson for animation) underneath the volatile UI; `modern-design.md` filters current web-design trend noise for what's evidenced to hold up in production versus what breaks on real devices, plus a web→app adaptation table. A routing table sends anything already owned by another skill — deeper taste work, Flutter tokens, data viz, deployment — to that skill instead of duplicating it.

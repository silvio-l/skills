...
Most of these skills are project-specific (game asset pipelines, Godot, app icons, SEO/ASO, ...) — only install `-g` (global) the ones you want available in *every* session; install the rest per-project with `-s <skill-name>` and no `-g`. See the scope tag on each skill below.
...
Each skill is tagged **global** (installed with `-g`, available in every session regardless of project) or **project-local** (installed per-project via `add -s <skill-name>`, no `-g`) — see `CLAUDE.md` → "Skill scope" for the full rationale and the exact install command.
...
**The Fix.** A manually-invoked creative-direction skill that gathers existing brand/product context from the repo, forces an explicit PRODUCT/BRAND/HYBRID strategy choice with stated reasoning, triages which brand assets are actually icon-capable, and develops three genuinely distinct concepts (not three variations) scored against recognizability, distinctiveness, and brand/product fit before recommending one. Concept and master-art generation are delegated to a Fable subagent — this skill's own explicit standing default for visual creation — while this skill itself plans and judges, it doesn't design inline. From the finished master SVG, a bundled script deterministically produces the actual platform asset sets — iOS (App Store Connect), macOS (`.icns` via `iconutil`), Windows (`.ico`, plus an opt-in full Microsoft Store MSIX asset set), and Android (launcher mipmaps, adaptive icon, Play Console listing icon) — each at the sizes and formats each store's own current docs specify (e.g. Play Console's listing icon needs alpha, Apple's must not), then self-validates that every required file actually exists at the correct pixel dimensions before anything is reported done. A second, fresh Fable subagent reviews rendered previews (smallest sizes especially) against the validation checklist as a final quality gate. For Expo/RN projects, hands the master SVG to the `app-icon` skill instead, which is already wired into Expo's asset pipeline.

### `fetch-shared-chat` — global

**The Problem.** Shared ChatGPT/Gemini/Claude.ai chat links are client-rendered SPAs — a plain `curl` or `WebFetch` returns an empty app shell, not the conversation, so the obvious approach silently fails. Worse, ChatGPT's DOM-rendered page client-side-redirects to its homepage both when a share is genuinely inaccessible *and* when nothing is actually wrong, making the two cases indistinguishable without digging further — and a naive fix risks fabricating a summary of content nobody ever actually read. And once the text is actually retrieved, "summarize it" throws away exactly the reasoning, decisions, and dead ends a user often wants to keep and continue working from later.

**The Fix.** A model-invoked skill that dispatches on the share URL's host. ChatGPT goes through its React Router single-fetch `.data` endpoint via plain HTTP — no browser at all — decoded by a from-scratch, cross-validated reimplementation of the `turbo-stream` wire format, which additionally surfaces the *exact* server-side reason a share failed (deleted vs. access-denied vs. workspace-scoped) instead of the ambiguous DOM redirect. Gemini falls back to headless-Chromium rendering, since no server-side data endpoint is known for it. Claude.ai is attempted the same way but currently hits a Cloudflare bot challenge, which the skill detects and reports explicitly rather than passing through as fake content. Every failure path returns a concrete, worded reason instead of an empty or fabricated result. Once fetched, the skill picks between two response modes: a quick inline answer/summary for a specific question, or — when the user wants the work preserved and continuable — a knowledge-base mode that distills the transcript into a structured Markdown file organized by topic rather than timeline (current state, per-topic facts/decisions/results, decision and change history, open points, unresolved contradictions), tagging every claim's epistemic status (user statement vs. verified fact vs. assumption vs. assistant proposal) and refusing to paper over superseded or contradicting statements, delivered as a file via `SendUserFile` rather than pasted inline.

### `fetch-open-chat-tab` — global

**The Problem.** Not every conversation can become a `fetch-shared-chat` share link — sharing can be off, or the conversation is one the user never intends to share. The chat still exists, live and fully rendered, in whatever Safari tab has it open — but that content only exists as JavaScript-rendered DOM in an already-authenticated session, invisible to any network-level fetch.

**The Fix.** A model-invoked skill that drives the user's own Safari tab directly via `do JavaScript` (through a small JXA driver), reading the real DOM — never a screenshot or OCR. Built against three live conversations (one per site) rather than assumed selectors, which surfaced two real bugs a naive implementation would have shipped silently: Claude.ai's `[data-test-render-count]` looked assistant-specific but actually tags *every* turn's markdown rendering, so an early version double-counted each user message as a duplicate "assistant" entry — fixed by keying off `[role="article"][aria-posinset]` instead, a stable per-turn position that also fixes collection order once claude.ai's (and likely gemini.google.com's) DOM virtualization is accounted for: long conversations un-mount off-screen messages, so a single DOM snapshot silently misses most of a long chat, requiring a scroll-to-top-then-sweep collection loop instead. A second bug — JXA's Standard Additions `read` command silently mangling non-ASCII text into mojibake — was caught the same way and fixed via the Foundation/NSString bridge for guaranteed UTF-8. An unstructured `body.innerText` fallback (clearly labeled, distinct exit code) covers any site without a dedicated extractor rather than failing outright.

### `seo-aso-optimizer` — project-local
...
### `marketing-video-automation` — project-local
...
### `name-clearance-red-team` — global
...
**The Problem.** A TP-Link Omada SDN controller exposes two different HTTP API surfaces (OpenAPI v1 and Web API v2) with different auth flows, and its actual base paths and resource names don't match the patterns a naive read of the web UI or older documentation would suggest — a base path with a subtly wrong path segment (singular vs. plural) returns a generic "unsupported path" error indistinguishable from a genuinely missing feature, and DHCP reservations aren't a listed resource at all but a field buried on each client.

**The Fix.** A model-invoked skill documenting both API surfaces from paths verified against a live controller rather than guessed from the UI, including the corrected site-scoped base path, the confirmed working DHCP-reservation read/write recipe (`clients/{mac}.ipSetting`, verified end-to-end including a real two-NIC reservation swap), and an honest split between confirmed-working endpoints and unverified-but-documented payload shapes rather than presenting both with equal confidence. Deliberately excludes browser-DevTools-based endpoint discovery, since a future agent instance typically has no browser access to fall back on.

### `godot-cli` — project-local
...
### `blender-scripting` — project-local

**The Problem.** Generating a 3D model with an LLM tempts two failure modes: leaning on a live MCP session (adds a server, times out on real exports, and couples every step to an open Blender instance) or treating "3D modeling" as pure geometry math and shipping something that's technically valid but reads as generic — repetitive scatter, random-angle branches, uniform noise on every rock regardless of what it's supposed to be made of. Blender's own Python API compounds this with real version drift (4.0 replaced direct node-socket creation with the `NodeTreeInterface` API, 5.x quietly removed the `mod["Input_N"]` pattern for Geometry Nodes modifier inputs), so code copied from an old tutorial can fail in a way that looks like a Blender bug rather than a stale assumption.

**The Fix.** A headless-only skill — `blender --background --python script.py`, no MCP server, no live session — built from research verified empirically against a real local Blender install rather than trusted from search results alone (several forum-era assumptions about `bpy.ops` failing in background mode turned out to be wrong on 5.2). A dedicated `design-sense.md` translates game-design and architecture principles — silhouette-first, primary/secondary/tertiary form order, biologically-grounded tree growth, geologically-grounded rock vocabulary, power-law scatter sizing, proportion systems — into one-line `bpy` consequences, read before generating new geometry and skipped for export/debug work. Every code example across the skill, including the ones illustrating these principles, is a tested, headlessly-run snippet, not copied pseudocode. Replaces the MCP-based `blender-mcp` skill entirely — its non-duplicative content (GLTF material-loss matrix, error table, `gltf-transform` pipeline) was absorbed into `export-and-errors.md` rather than lost.

### `game-asset-director` — project-local

**The Problem.** `blender-scripting` generates strong procedural hard-surface geometry but produces generic, blocky results for hero and organic assets — exactly the assets that most need to look AAA — and has no answer at all for 2D sprites, icons, or UI art. Treating "generate a game asset" as one uniform pipeline either wastes AI-mesh budget on rocks and buildings pure `bpy` already handles well, or ships an organic character straight from a naive Decimate pass — which, per real production testing, corrupts the texture rather than just simplifying the mesh — without ever checking the result against a mobile game's actual polycount and texture budget before calling it done.

**The Fix.** A director that classifies each asset request — 3D or 2D — and routes it automatically: procedural/parametric hard-surface work (rocks, terrain, modular architecture, generative props) goes straight to `blender-scripting`, unmodified; hero/organic 3D work goes through fal.ai's flagship image-to-3D tier (Hyper3D Rodin's quad-topology + PBR output, or Meshy, both reachable through the one fal.ai account already in use — no separate signups); 2D sprites, icons, and UI art go through fal.ai image generation, background removal, and upscaling. Every hero-tier AI-sourced 3D mesh runs through this skill's own finishing pipeline — duplicate the high-poly result, re-UV a low-poly copy with Smart UV Project, and bake the high-detail normal/AO maps onto it, the technique that actually avoids the texture-corruption failure a bare Decimate produces — before a chained-Decimate LOD pass and the final export, which is handed back to `blender-scripting`'s version-safe export code. Self-validating scripts check every 3D and 2D result against mobile budgets, topology/UV integrity, and canvas/alpha correctness before anything is reported done; fixable failures retry automatically (bounded), but a defect needing a new paid generation stops and asks rather than silently spending again.

### `instructions-optimizer` — global

**The Problem.** A bloated `CLAUDE.md`/`.cursorrules` wastes context on every session, but "just delete the fluff" invites two opposite failure modes: an agent doing it ad hoc improvises its own output format and can delete a load-bearing rule along with the filler, while an agent trying to automate the cut with regex can't tell general knowledge from a hard project rule — that distinction needs reading comprehension, not pattern matching. A first version of this skill tried exactly that: a regex engine with a `## Overview` section-strip pattern that silently no-offed (missing `re.DOTALL`), file discovery that probed both `CLAUDE.md` and `claude.md` as separate targets and double-processed the same file on any case-insensitive filesystem, and `.claude.json` listed as a compression target — Claude Code's live session state, not instruction prose.
...
**The Fix.** A thin, deterministic workflow: pull metadata first (`--dump-json`) to decide whether manual subtitles exist before falling back to auto-captions, fetch the transcript, then always pipe the resulting `.vtt` through a bundled stdlib-only script (`scripts/clean_vtt.py`) that strips the timing tags and drops lines already emitted by a recent cue (a small trailing window, not just "equals the previous line" — a cue can carry over more than one prior line unchanged) rather than ever reading the raw file — verified against a real video end-to-end via n-gram redundancy checks, plus its error paths (missing file, empty file). An optional `--timestamps` mode emits `[mm:ss] text` per line for tasks that need to locate a specific moment, not just read the gist. A short pitfalls section covers the harmless `impersonation` warning, `--skip-download` still fetching a tiny metadata format, and mapping common `yt-dlp` failures (private/deleted/region-locked video, no captions available) to a clear message instead of a raw traceback.

### `motion-graphics-director` — project-local

**The Problem.** Turning a video's raw transcript into animated marketing graphics tempts an agent to animate every sentence (nobody wants that) and to invent plausible-looking numbers when the speaker states a stat without a source — exactly the kind of confident-but-wrong output that's worse than no graphic at all. Left to its own defaults, the model also reaches for the same generic AI-slop look — Inter, an ungrounded color guess — on every graphic, so a set of graphics from the same project never reads as one coherent visual system, which was the single most-upvoted complaint on the YouTube video this skill's workflow was reverse-engineered from.

**The Fix.** A seven-step pipeline verified against that real video's own transcript: get a `[mm:ss]`-timestamped transcript via the sibling `youtube-transcript` skill, editorially select the handful of moments actually worth animating (a stat, a comparison, a sharp claim — not everything), verify each one's real numbers via web research with a hard no-fabrication rule (skip the moment or mark it illustrative rather than invent a figure), then dispatch each graphic's actual design and build to `fable` — this skill's explicit standing UI model — as a Claude Artifact pulling type and color from a `MOTION-DESIGN.md` project doc built once and reused across every future video rather than re-derived per run. Before publishing, each graphic is checked against a concrete anti-slop list (decorative grid backgrounds, the purple-gradient reflex, decorative numbered markers, uniform scattered micro-animation instead of one orchestrated moment, default borders, hierarchy drift) and against the rendered result itself — reduced-motion respected, animation watched through, not approved from the markup. Deliberately thin on mechanism of its own — it loads and defers to this repo's existing `dataviz` skill for color/chart methodology and `artifact-design`/`artifact-diagramming` for Artifact fundamentals rather than re-implementing any of them, and explicitly scopes out local-footage input and video-compositing output for v1 (documented extension points) rather than guessing at unverified mechanisms.

### `landing-page-director` — project-local

**The Problem.** "Build me a landing page" tempts a model straight into the same generic AI-design defaults regardless of what the actual product is — a purple gradient on cream, decorative grid-line hatching behind ordinary content, a stock device mockup instead of the product's real differentiator, invented metrics and testimonials — and nothing forces it to ground the page in the project's actual brand facts (real logo file, real color tokens, real product truth) before designing, or to actually look at the rendered result before calling it done.

**The Fix.** A sequencing skill, not a competing design methodology: it loads Anthropic's own `frontend-design` skill first for the actual typography/color/motion craft, adds a brand-facts-gathering step that reads real source files (the actual mark SVG, the actual token file — which wins over a written brand doc when they disagree) instead of inventing identity, a brief-structure template for dispatching the creative pass to `fable` as this skill's explicit standing UI model (dropping back to the current tier once the direction is locked for mechanical edits), a concrete anti-slop checklist of the specific patterns that read as generated rather than designed, and a verification step that actually opens the result in a browser — checking `prefers-reduced-motion`, responsive behavior, and both color schemes — rather than approving from the markup. Generalized from a project-specific skill built for one product's brand rules into a project-agnostic version any product can point at its own facts.

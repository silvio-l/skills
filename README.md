...
**The Fix.** A five-step pipeline verified against that real video's own transcript: get a `[mm:ss]`-timestamped transcript via the sibling `youtube-transcript` skill, editorially select the handful of moments actually worth animating (a stat, a comparison, a sharp claim — not everything), verify each one's real numbers via web research with a hard no-fabrication rule (skip the moment or mark it illustrative rather than invent a figure), then generate each graphic as a Claude Artifact pulling type and color from a `MOTION-DESIGN.md` project doc built once and reused across every future video rather than re-derived per run. Deliberately thin on mechanism of its own — it loads and defers to this repo's existing `dataviz` skill for color/chart methodology and `artifact-design`/`artifact-diagramming` for Artifact fundamentals rather than re-implementing any of them, and explicitly scopes out local-footage input and video-compositing output for v1 (documented extension points) rather than guessing at unverified mechanisms.

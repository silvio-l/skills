---
name: deep-monograph
description: Produce a deep, well-researched, fact-checked long-form monograph on a complex topic via a clarify-research-write-verify pipeline. Use for 'Fachartikel', 'Fachmonografie', 'Hintergrundartikel', 'Dossier', 'Ratgeber', deep-dive explainer.
---
...
This pipeline is for book-chapter-depth, multi-thousand-word works — not for a quick answer, a short blog post, a single how-to, or a simple summary. If the actual request is for something shorter than the "thorough article" depth level (see `capabilities-and-modes.md`), just answer directly instead of invoking this heavyweight clarify-research-write-verify machinery.

## Output language

Default to **German** for the final monograph and all reader-facing artifacts (reader's guide, glossary, comprehension check, etc.) — the process this skill implements was authored for a German-speaking home-user audience and assumes German output unless the user explicitly asks for another language. This skill's own documentation (this file and its supporting files) stays in English per repo convention; only the produced artifact defaults to German.
...
3. **Evidence before claims.** Every material factual claim is typed (verified finding / manufacturer claim / experience value / plausible inference / editorial recommendation / open question) and, where it matters, checked against an independent source. Never manufacture sources, quotes, URLs, or figures. See `research-and-sources.md`.
...
The full procedure is a sixteen-phase pipeline — order the work, don't skip steps. See `process.md` for the phase-by-phase breakdown; each phase links to the file that details it.
...
4. Build a contradiction-free mental model, design the chapter dramaturgy — **in agentic workspace mode, a second hard gate: wait for explicit outline approval** — and write (`didactics-and-writing.md`).
5. Fact-check, technically review, critically counter-check, pedagogically review, practically review, copy-edit (`quality-and-output.md`).
...
| `process.md` | The 16-phase pipeline, phase by phase |

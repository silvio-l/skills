...
# StoreEdge is the Microsoft Store client's own product API — clean JSON with
# AverageRating / RatingCount / Description / Categories, far more reliable than
# scraping the apps.microsoft.com SPA DOM (whose rating selectors had rotted, so
# every entry came back with rating 0). Treated as an official store API (same
# class as iTunes Lookup / Play): politeness is enforced (rate-limit + cache),
# robots is not gated on the store's own product endpoint.
_STOREEDGE_PRODUCT_URL = "https://storeedgefd.dsx.mp.microsoft.com/v9.0/products/{pid}"
_API_RATE = POLITE.RateLimiter(seed=23)
...
def _storeedge_market_locale(country: str, language: str):
    """(market, locale) for the StoreEdge product API, e.g. ('DE', 'de-de')."""
    market = (country or "us").upper()
    lang = (language or country or "en").lower()
    return market, f"{lang}-{market.lower()}"


def _parse_storeedge_product(payload) -> Optional[Dict]:
    """Pull the fields we keep from a StoreEdge product payload (clean JSON)."""
    if not isinstance(payload, dict):
        return None
    p = payload.get("Payload") if isinstance(payload.get("Payload"), dict) else payload
    if not isinstance(p, dict) or not (p.get("Title") or p.get("Description")):
        return None
    cats = p.get("Categories") or []
    rating_count = p.get("RatingCount")
    return {
        "title": p.get("Title") or "",
        "description": schema.strip_html(str(p.get("Description") or "")),
        "short_description": schema.strip_html(str(p.get("ShortDescription") or "")),
        "rating_avg": p.get("AverageRating"),
        "rating_count": int(rating_count) if isinstance(rating_count, (int, float)) else 0,
        "category": (cats[0] if cats else None),
        "last_updated": p.get("LastUpdateDateUtc"),
        "publisher": p.get("PublisherName") or p.get("Publisher"),
        "reviews_sample": [],  # the product API returns aggregate ratings, no review text
    }


def fetch_storeedge_product(
    app_id: str,
    *,
    market: str,
    locale: str,
    ...
    """One StoreEdge product-API call, cache-backed. None on any failure."""
    url = _STOREEDGE_PRODUCT_URL.format(pid=app_id) + "?" + urllib.parse.urlencode(
        {"market": market, "locale": locale, "deviceFamily": "Windows.Desktop"}
    )
    key = CACHE.cache_key("GET", url, {})
    path = CACHE.cache_path(cache_dir, key)
    now_ts = time.time() if now is None else now

    if not fresh and cache_dir and CACHE.is_fresh(path, ttl, now_ts):
        cached = CACHE.read_cache(path)
        if cached is not None:
            try:
                return _parse_storeedge_product(json.loads(cached.decode("utf-8")))
            except Exception:
                pass
    ...
    for attempt in range(POLITE.MAX_RETRIES):
        try:
            ...
            with urllib.request.urlopen(req, timeout=30) as resp:
                ...
                raw = resp.read()
            if status in POLITE.RETRY_STATUS:
                time.sleep(POLITE.backoff_delay(attempt, rng=_API_RATE._rng))
                continue
            payload = raw
            break
        except Exception:
    ...
    if cache_dir:
        try:
            ...
            pass
    try:
        return _parse_storeedge_product(json.loads(payload.decode("utf-8")))
    except Exception:
        return None
...
def fetch_ms_detail(
    ...
    *,
    ...
    """Enrich a list of MS app IDs via the StoreEdge product API (HTTP, cache-backed).

    Returns ``{app_id: {description, short_description, rating_avg, rating_count,
    category, last_updated, publisher, reviews_sample}}``. Never raises — a
    missing entry means the product could not be resolved. No browser needed;
    the previous SPA-DOM scraper never recovered ratings.
    """
    result: Dict[str, Dict] = {}
    if not app_ids:
        return result
    market, locale = _storeedge_market_locale(country, language)
    for app_id in list(app_ids)[:cap]:
        try:
            detail = fetch_storeedge_product(
                app_id, market=market, locale=locale,
                cache_dir=cache_dir, ttl=ttl, now=now, fresh=fresh,
            )
        except Exception:
            detail = None
        if detail:

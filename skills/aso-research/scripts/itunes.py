...
def process_results(
    ...
    generics = [config.get("category", ""), "apple", "ios", "iphone", "ipad"]
    extracted = extract.extract_keywords(
        [c.get("title", "") for c in competitors],
        generics=generics,
    )
    keywords = score.score_keywords(
        extracted,
        seed_keywords=config.get("seed_keywords") or [],
...
def discover(
    ...
    *,
    ...
    return process_results(accumulated, config)

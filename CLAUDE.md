...
- **global:** `apple-notes`, `brainstorming`, `deep-monograph`, `escalate`, `fetch-open-chat-tab`, `fetch-shared-chat`, `name-clearance-red-team`, `omada-admin`, `owasp-bsi-audit` (the last is an explicit standing exception, see the global `~/.claude/CLAUDE.md`)
